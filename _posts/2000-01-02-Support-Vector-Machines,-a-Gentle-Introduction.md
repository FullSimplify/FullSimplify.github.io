...
Such kernel is necessarily **positive definite**. Moreover, *every reproducing kernel can be represented with an inner product of feature maps.* We have reached the conclusion that we can represent a reproducing kernel in a RKHS with the formula above.

### Summary

We have seen that every RKHS has a unique reproducing kernel which is also positive definite. Such kernel can be represented with the inner product of the RKHS. We add one piece to the puzzle. Thanks to <a target="_blank" href='https://en.wikipedia.org/wiki/Reproducing_kernel_Hilbert_space#Moore.E2.80.93Aronszajn_theorem'> Moore-Aronszain</a> theorem, every positive definite kernel is a reproducing kernel. To summarize:

Given a RKHS, $$\mathcal{H}$$ $$\Rightarrow$$ it is possible to define a reproducing kernel with the inner product in $$\mathcal{H}$$

$$\forall$$ positive definite kernel $$\Rightarrow$$ it is a reproducing kernel $$\Rightarrow$$ (remember the "if and only if" above) there is a unique RKHS with $$\kappa$$ being its reproducing kernel.
